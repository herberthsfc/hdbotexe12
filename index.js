			...
			case 'mapa': 
				...
				case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`❬ 💟 ❭ Para criar uma figurinha, mande uma foto,gif ou video de até 5 segundos, e digite ${prefix}figu na legenda!`)
					}
					break
				case 'gtts':	
				case 'tts':
				case 'audio':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/NfdgBqz.jpg`)
